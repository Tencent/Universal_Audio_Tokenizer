...
class ResBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.activations1 = nn.ModuleList([
            ...
            for _ in range(len(self.conv1))
        ])
        self.activations2 = nn.ModuleList([
            Snake(channels, alpha_logscale=False)
            for _ in range(len(self.conv2))
        ])

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        for idx in range(len(self.conv1)):
            xt = self.activations1[idx](x)
            xt = self.conv1[idx](xt)
            xt = self.activations2[idx](xt)
            xt = self.conv2[idx](xt)
            x = xt + x
        return x

    def remove_weight_norm(self):
        for idx in range(len(self.conv1)):
            remove_weight_norm(self.conv1[idx])
            remove_weight_norm(self.conv2[idx])
...
class HiFTGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.post_conv = weight_norm(Conv1d(ch, istft_params["n_fft"] + 2, 7, 1, padding=3))
