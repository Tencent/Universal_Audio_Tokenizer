...
class WhisperGenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        # Handle different encoder types (Whisper vs Qwen3Omni)
        if hasattr(self.model.encoder, 'conv1') and hasattr(self.model.encoder, 'conv2'):
            # Whisper encoder: simple stride calculation
            input_stride = self.model.encoder.conv1.stride[0] * self.model.encoder.conv2.stride[0]
        elif hasattr(self.model.encoder, 'conv2d1') and hasattr(self.model.encoder, 'conv2d2') and hasattr(self.model.encoder, 'conv2d3'):
            # Qwen3Omni encoder with 3 conv layers
            input_stride = self.model.encoder.conv2d1.stride[0] * self.model.encoder.conv2d2.stride[0] * self.model.encoder.conv2d3.stride[0]
        else:
            raise ValueError("Unknown encoder type: cannot determine input_stride from convolution layers")

        num_segment_frames = input_stride * self.config.max_source_positions
        batch_size, total_input_frames = self._retrieve_total_input_frames(
            input_features=input_features, input_stride=input_stride, kwargs=kwargs
        )
        # print(f'total_input_frames', total_input_frames)
    ...
    def generate_with_fallback(
        ...
        for fallback_idx, temperature in enumerate(temperatures):
            ...
            assert segment_input is not None, "segment_input is None"
            # print(f'generation_config: {generation_config}')
            # print(f'generate_kwargs: {generate_kwargs}') # attention_mask:tensor[[]]
            # generation_config.pop("forced_decoder_ids")
