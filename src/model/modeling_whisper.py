...
class QuantizedBaseModelOutput(BaseModelOutput):
    ...
    num_update_indices: Optional[int] = None
    quantized_loss: Optional[torch.FloatTensor] = None
    codebook_usage: Optional[float] = None
    residual_gamma_mean: Optional[torch.FloatTensor] = None
...
def masked_mean_pooling(hidden_states, attention_mask, dim=1):
    """
    对hidden_states在指定维度进行masked mean pooling

    Args:
        hidden_states: [batch_size, seq_len, hidden_dim]
        attention_mask: [batch_size, seq_len] - True for valid tokens, False for padding
        dim: 要进行pooling的维度，默认为1（序列维度）

    Returns:
        pooled_states: [batch_size, hidden_dim] - 经过mask pooling后的结果
    """
...
class WhisperVQEncoder(WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperVQConfig):
        ...
        if self.config.quantize_encoder_only:
            self.layers = nn.ModuleList([WhisperVQEncoderLayer(
                self.config,
                is_causal=self.config.encoder_causal_attention or self.config.quantize_causal_encoder
            ) for _ in range(min(self.config.quantize_position, self.config.encoder_layers))])
        else:
            self.layers = nn.ModuleList([WhisperVQEncoderLayer(
                self.config,
                is_causal=self.config.encoder_causal_attention or (
                        self.config.quantize_causal_encoder and layer_id < self.config.quantize_position
    ...
    def _init_quantize_layer(self):
        if self.config.quantize_vocab_size is not None:
            ...
            self.codebook = nn.Embedding(self.config.quantize_vocab_size, self.config.d_model)
            if self.config.codebook_gaussian_init:
                self.codebook.weight.data.copy_(
                    torch.randn(self.config.quantize_vocab_size, self.config.d_model) * self.config.d_model ** -0.5
                )
            ...
            if self.config.quantize_ema_decay is not None:
                ...
                self.register_buffer("ema_count", torch.ones(self.config.quantize_vocab_size, dtype=torch.float))
    ...
    def _init_codebook_usage_buffer(self):
        if self.config.quantize_vocab_size is not None:
            ...
            print(f"Codebook usage tracking buffer size: {self.codebook_usage_buffer_size}")
        else:
            self.codebook_usage_buffer_size = None

    def _init_residual_adapter(self, dropout=0.1):
        """初始化自适应残差连接的模块"""
        if self.config.encoder_residual_source_position is not None \
            and self.config.encoder_residual_target_position is not None:
            hidden_dim = self.config.d_model

            # MLP_Adapter: 将低层特征适配到高层
            ...
            self.residual_output_norm = nn.LayerNorm(hidden_dim)
    ...
    def _apply_gated_residual_connection(self, x_low, x_high, attention_mask=None):
        """
        x_low:  低层特征 (batch_size, seq_length, hidden_dim)
        x_high: 高层特征 (batch_size, seq_length, hidden_dim)
        attention_mask: (batch_size, seq_length), 1 for valid positions, 0 for masked
        """
        if self.config.use_residual_adapter:
            residual = self.residual_adapter(x_low)
        else:
            residual = x_low

        if self.config.use_residual_gate:
            gate_input = torch.cat([x_high, x_low], dim=-1)     # (batch_size, seq_length, hidden_dim * 2)
            gamma_low = self.residual_gate(gate_input)          # (batch_size, seq_length, hidden_dim)
        else:
            gamma_low = torch.tensor(1.0, device=x_high.device)

        output = x_high + gamma_low * residual
        output = self.residual_output_norm(output)  # (batch_size, seq_length, hidden_dim)

        if attention_mask is not None and isinstance(gamma_low, torch.Tensor) and gamma_low.dim() > 0:
            # attention_mask: (batch_size, seq_length) -> (batch_size, seq_length, 1)
            mask = attention_mask.unsqueeze(-1)  # (batch_size, seq_length, 1)
            gamma_abs = gamma_low.abs() * mask
            gamma_mean = gamma_abs.sum() / (mask.sum() * gamma_low.shape[-1] + 1e-8)
        else:
            gamma_mean = gamma_low.abs().mean()

        return output, gamma_mean


    def _apply_vq_quantization(self, hidden_states, attention_mask, batch_size, quantized_token_ids=None):
        """应用 VQ 量化"""
        if quantized_token_ids is not None:
            ...
            if self.training:
                ...
                if self.config.quantize_ema_decay:
                    ...
                    self.quantize_loss = self.config.quantize_loss_scale * self.config.quantize_commit_coefficient * \
                                         mse_loss_with_mask(hidden_states, hidden_quantized.detach(), attention_mask)

                    self.quantize_ema_count += 1
                    if self.config.quantize_restart_interval is not None and \
                       self.quantize_ema_count % self.config.quantize_restart_interval == 0:
                        ...
                        # .nonzero() returns the indices of elements that are non-zero, with shape (num_nonzero, 1)
                        ...
                        hidden_update = hidden_selected[random.choices(range(len(hidden_selected)), k=num_update)]
                        ...
                        if torch.distributed.get_rank() == 0:
                            ...
                            self.num_update_indices = len(update_indices)
                else:
                    # print(f'is self.codebook.weight.requires_grad: {self.codebook.weight.requires_grad}')
                    input_to_quantized_loss = mse_loss_with_mask(
                        hidden_states,
                        hidden_quantized.detach(),
                        attention_mask
                    )
                    quantized_to_input_loss = mse_loss_with_mask(
                        hidden_quantized,
                        hidden_states.detach(),
                        attention_mask
                    )
                    loss = self.config.quantize_loss_scale * \
                           (self.config.quantize_commit_coefficient * input_to_quantized_loss + quantized_to_input_loss)
                    self.quantize_loss = loss

                hidden_states = hidden_states + (hidden_quantized - hidden_states).detach()

                all_quantized_token_ids = quantized_token_ids.reshape(-1)
                valid_tokens = all_quantized_token_ids[attention_mask.reshape(-1) > 0]
                if valid_tokens.size(0) > self.codebook_usage_buffer_size:
                    valid_tokens = valid_tokens[:self.codebook_usage_buffer_size]  # Truncate to buffer size
                ...
                if torch.distributed.is_initialized():
                    ...
                    torch.distributed.all_gather(
                        size_list, torch.tensor(unique_list_size, device=hidden_states.device)
                    )
                    unique_list = [torch.zeros(
                        size.item(), dtype=local_unique.dtype, device=hidden_states.device
        ...
        return hidden_states, quantized_token_ids, codebook_usage
    ...
    def forward(
        ...
        #     print(f"=== Encoder input hidden_states shape: {hidden_states.shape} ===")
        ...
        assert attention_mask.shape[-1] == hidden_states.shape[1], \
            f"attention_mask.shape[-1]: {attention_mask.shape[-1]}, hidden_states.shape[1]: {hidden_states.shape[1]}"
        ...
        if self.config.pooling_position == 0 and self.config.pooling_kernel_size is not None:
            ...
            if self.config.quantize_causal_block_size is not None:
                extended_attention_mask = self.get_block_causal_attention_mask(
                    attention_mask,
                    block_size=self.config.quantize_causal_block_size // self.config.pooling_kernel_size
                )
            else:
        ...
        # if torch.distributed.is_initialized() and torch.distributed.get_rank() == 0:
        #     print(f"=== Encoder hidden_states shape after initial pooling: {hidden_states.shape} ===")
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if idx + 1 == self.config.pooling_position and self.config.pooling_kernel_size is not None:
                ...
                if self.config.quantize_causal_block_size is not None:
                    ...
                    extended_attention_mask = self.get_extended_attention_mask(
                        attention_mask, batch_size, seq_length // self.config.pooling_kernel_size
                    )

            if self.config.encoder_residual_source_position is not None \
                and idx + 1 == self.config.encoder_residual_source_position:
                residual = hidden_states

            if self.config.encoder_residual_target_position is not None \
                and idx + 1 == self.config.encoder_residual_target_position:
                if residual is not None:
                    hidden_states, gamma = self._apply_gated_residual_connection(
            ...
            if idx + 1 == self.config.quantize_position and self.config.quantize_vocab_size is not None:
                hidden_states, quantized_token_ids, codebook_usage = self._apply_vq_quantization(
                    hidden_states, attention_mask, batch_size, quantized_token_ids
                )
        ...
        return QuantizedBaseModelOutput(
            ...
            num_update_indices=self.num_update_indices,
            quantized_loss=self.quantize_loss,
            codebook_usage=codebook_usage,
            residual_gamma_mean=gamma,
        )
    ...
    def forward(
        ...
        Args:
            ...
            attention_mask (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                ...
                [What are attention masks?](../glossary#attention-mask)
            encoder_hidden_states (`torch.FloatTensor` of
                shape `(batch_size, encoder_sequence_length, hidden_size)`, *optional*):
        ...
        if input_ids is not None:
            # print('self.max_target_positions', self.max_target_positions)
...
class WhisperVQModel(WhisperPreTrainedModel):
    ...
    def forward(
        ...
        Example:
         ...
         ```"""
        # batch_size, feature_size, seq_length = input_features.shape
        # note that input_features can be none, if encoder_outputs is passed
        # if attention_mask is None:
        #     attention_mask = torch.ones(
        #         batch_size, seq_length, dtype=input_features.dtype, device=input_features.device
        #     )
        ...
        return QuantizedSeq2SeqModelOutput(
            ...
            num_update_indices=encoder_outputs.num_update_indices,
            quantized_loss=encoder_outputs.quantized_loss,
            codebook_usage=encoder_outputs.codebook_usage,
            quantized_token_ids=encoder_outputs.quantized_token_ids,
            residual_gamma_mean=encoder_outputs.residual_gamma_mean,
        )
...
class WhisperVQForConditionalGeneration(WhisperGenerationMixin, WhisperPreTrainedModel):
    ...
    def forward(
        ...
        # assert input_features is not None and attention_mask is not None, "input_features or attention_mask is None"
        outputs = self.model(
            input_features,
            attention_mask=attention_mask,
        ...
        if labels is not None:
            ...
            loss = ce_loss
            if self.training:
                if self.config.use_commit_loss and outputs.quantized_loss is not None:
                    loss = loss + outputs.quantized_loss

        if not return_dict:
            output = (lm_logits,) + outputs[1:]
            return ((loss,) + output) if loss is not None else output

        return QuantizedSeq2SeqLMOutput(
            loss=loss,
            ce_loss=ce_loss,
            ...
            quantized_token_ids=outputs.quantized_token_ids,
